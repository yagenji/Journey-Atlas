...
| Open PRs #610, #647, #651, #656, #714, #715 | Not closed by this cleanup. Compare each unique diff against latest main and check active work before deciding whether it is superseded |
...
- An exact full-path image search is not sufficient to prove an orphan:
  `world-map-watercolor.svg` is referenced through a relative CSS URL, and the
  packager includes files under `assets/images/top/` as a directory dependency.
  No top-page asset is removed by this change.
