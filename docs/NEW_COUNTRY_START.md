...
- Travel Scale（旅の目安日程）は3段階とも**日数表示を残します**。各国の規模・見どころの分散・移動負荷に合わせて `3〜4日` / `7〜10日` のような日数レンジを設定し、第3段階は `○日以上` とします。
- Travel Scaleは3段階すべてに具体的な `例：` を必須とします。
- **日数・泊数・週数を禁止するのは `例：` より後ろのルート例の中だけです。** 例の中に `Aを2日` / `3泊` / `1週間` のような滞在日数を入れてはいけません。
- `例：` は代表的な旅の流れだけを示します。例：`A → B → C`。日数は上の `duration` で示し、例の中では繰り返しません。
