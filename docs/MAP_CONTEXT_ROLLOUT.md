...
## Opt-in shared generation and update

For a **new** single-region Country, `scripts/generate_country_map_with_context.py` orchestrates the existing `generate_country_map.py` and `add_country_map_context.py`, using bounds from Country JSON and a **target-country-specific** Natural Earth or geoBoundaries boundary source. GSHHS coastlines alone cannot identify the target country's political boundary, so the wrapper intentionally rejects GSHHS as a target source. Example:
...
For an **existing approved SVG**, `scripts/add_country_map_context.py` derives full-canvas geographic bounds from the unchanged local-equirectangular fit, uses installed GSHHS polygons via Basemap, preserves the target-country SVG path bytes and writes a **separate preview**:
...
Generic filenames illustrate the command; do not create a Country called `example`. Inspect source-data metadata, the exact country shape, shoreline detail, the actual rendered preview, and all markers before replacing any production asset. The existing Azerbaijan map already has context; do not run the stage again over that asset.

## Explicit non-goals and blockers during preparation

- The context stage currently accepts one untransformed target `<path fill="url(#land)">` with the canonical local-equirectangular projection. It **rejects** transformed SVG paths and `map.regions` multi-region maps, including United States-style insets. Those maps require a reviewed, shared region-aware implementation and QA before migration. Do not flatten regions or use fake geographic coordinates.
...
- Existing maps, Country JSON, Production States, publication flags, sitemap, and discovery links remain unchanged on this preparation branch. The current default map generator is not modified.
...
6. After shared behavior is verified, update the human new-Country procedure to require the shared generator with context at the CONTENT + MAP stage. Do not silently migrate in-flight legacy Countries.
