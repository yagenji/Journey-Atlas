...
- **Monaco real-land comparison, 2026-09-19:** [diagnostic-only preflight and original-size SVG/PNG artifact](https://github.com/yagenji/Journey-Atlas/actions/runs/35436523029/artifacts/10582686018) PASS. Five intersecting existing French *commune polygons* from the Alpes-Maritimes were clipped in the same WGS84 canvas, replacing **only the separately generated geographic-context path**. All original non-context SVG markup, including the approved Monaco silhouette, was identical and both 1200×760 PNGs fully decoded. This removes the unsupported frame-crossing GSHHG pentagon and shows a more detailed French coastal context, but residual seams at the Monaco boundary and source-vintage differences remain; **visual/geographic HOLD is not lifted**. Source is the third-party [gregoiredavid/france-geojson municipality file](https://github.com/gregoiredavid/france-geojson/blob/master/departements/06-alpes-maritimes/communes-06-alpes-maritimes.geojson), source SHA-256 `eda4ff9e2d35a6c24d9d0f12dbcb2f6faeb3e2fa6f114ef80d22aa586dc3524e`. The [source README](https://github.com/gregoiredavid/france-geojson/blob/master/README.md) documents **2018 IGN Admin Express input**, a **25% simplified** derivative and France Open Licence. A valid 2018 polygon is useful comparison evidence, **not a current or approved production coastline**; do not silently substitute it into the rollout or turn this one-off QA step into a permanent fetch dependency.
...
- **Monaco:** the [Shom/IGN Limite terre-mer reference](https://www.data.gouv.fr/datasets/limite-terre-mer-1) supplies a high-resolution coastal line under France's Open Licence 2.0; [IGN explicitly includes Monaco in its coverage](https://www.geoportail.gouv.fr/embed). It is a line reference, **not a preverified fill polygon**. For a newer real French land polygon, [Etalab's official administrative-contours dataset](https://www.data.gouv.fr/datasets/contours-administratifs) and [IGN Admin Express's 2025 coastline update](https://cartes.gouv.fr/aide/fr/partenaires/ign/generalites-ign/actualites/2025-05-admin-express-v4/) provide next-source leads; retrieve the actual nearby commune polygons, check precision, version and licence, and compare their interface with Monaco's approved target before adopting. The 2018 mirror comparison above does **not** settle this.
