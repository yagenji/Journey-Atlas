...
const DATA_VERSION = '03031826a626c65a';
const ICON_SPRITE = 'assets/icons/atlas-icons.svg';
const SITE_ORIGIN = 'https://yagenji.github.io/Journey-Atlas/reviews/stvincentgrenadines/';
...
function renderTaste(fragment, data = {}) {
  ...
  items.forEach((item) => {
    ...
    if (!item.image.endsWith('.parts.json') && !item.image.endsWith('.b64')) {
...
function renderTravelScale(fragment, data = {}) {
  const section = fragment.querySelector('#travel-scale-section');
  const grid = fragment.querySelector('#travel-scale-grid');
  if (!section || !grid || !Array.isArray(data.items) || !data.items.length) {
    if (section) section.hidden = true;
    return;
  }
  fragment.querySelector('#travel-scale-kicker').textContent = data.kicker || 'TRAVEL SCALE';
  fragment.querySelector('#travel-scale-title').textContent = data.title || '';
  const intro = fragment.querySelector('#travel-scale-intro');
  if (intro) {
  ...
  data.items.forEach((item) => {
    ...
    article.innerHTML = `<span class="travel-scale__icon">${iconSvg(item.icon || 'calendar')}</span><div><strong>${escapeHtml(item.duration || '')}</strong><h4>${escapeHtml(item.title || '')}</h4><p>${escapeHtml(item.text || '')}</p></div>`;
    grid.append(article);
  });
  section.hidden = false;
}
