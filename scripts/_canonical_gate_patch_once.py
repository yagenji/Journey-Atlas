#!/usr/bin/env python3
"""Temporary, exact-source patch. The workflow deletes this file before PR."""
from pathlib import Path

path = Path('scripts/publication_pipeline_v2.py')
text = path.read_text(encoding='utf-8')
start = text.index('def publish_ready_errors(')
end = text.index('\ndef increment_metric(', start)
segment = text[start:end]
needle = '''    preview = state.get("reviewPreview") if isinstance(state.get("reviewPreview"), dict) else {}
...
replacement = '''    preview = state.get("reviewPreview") if isinstance(state.get("reviewPreview"), dict) else {}
...
assert segment.count(needle) == 1, 'publish gate source mismatch'
text = text[:start] + segment.replace(needle, replacement) + text[end:]
old = '''    state["reviewDeployment"] = {
        "state": "DONE",
        "url": production_url,
        "productionVerification": "CI_GATED",
        "pipelineVersion": 2,
    }'''
new = '''    previous_review = state.get("reviewDeployment") or {}
...
assert text.count(old) == 1, 'finalizer source mismatch'
text = text.replace(old, new)
path.write_text(text, encoding='utf-8')
test = Path('scripts/test_canonical_review_publication_gate.py')
...
''', encoding='utf-8')
print('Guarded canonical-reviewed publication compatibility patch prepared')
