#!/usr/bin/env python3
"""Create a schema-v2 JOURNEY ATLAS country JSON scaffold from the canonical registry."""
...
def scaffold(destination: dict, region_label: str) -> dict:
    ...
    return {
        ...
        "taste": {
            "kicker": f"TASTE OF {name_en.upper()}",
            "title": f"{name_ja}で食べたいもの",
            "intro": "",
        ...
        "signatureFacts": [
            {"topicKey": "", "label": "", "value": "", "note": "", "icon": "", "interestReason": ""}
            for _ in range(3)
        ],
...
def main() -> int:
    ...
    parser.add_argument("slug", help="Destination slug already present in the canonical registry")
    ...
    output.write_text(
        json.dumps(scaffold(destination, region_label), ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )

    print(f"Created {output.relative_to(ROOT)}")
    print(f"Region taxonomy label: {region_label}")
    print("No Production State was created. Build the finished content/map, produce and approve 13 images, then implement once.")
    print("Finish-line QA: editorial + quality + schema + map + images + Desktop/Tablet/Mobile Browser QA.")
    return 0
