...
SOURCE_URL = (
    ...
    f"{SOURCE_COMMIT}/geojson/ne_10m_admin_0_sovereignty.geojson"
)
MARGIN_RATIO = 0.04
SIMPLIFY = 0.001
BAIKONUR_POINT = Point(63.38301549, 45.97819777)
...
def download_source() -> Path:
    target = Path(tempfile.gettempdir()) / "ne_10m_admin_0_sovereignty.geojson"
...
def country_geometry(dataset: Path):
    ...
    if "SOV_A3" in gdf.columns:
        selected = gdf[gdf["SOV_A3"].astype(str).eq("KAZ")]
    if selected.empty and "SOVEREIGNT" in gdf.columns:
        selected = gdf[gdf["SOVEREIGNT"].astype(str).str.casefold().eq("kazakhstan")]
    if selected.empty and "ADMIN" in gdf.columns:
        selected = gdf[gdf["ADMIN"].astype(str).str.casefold().eq("kazakhstan")]
    if selected.empty:
        raise SystemExit("Kazakhstan sovereignty geometry not found in Natural Earth 1:10m dataset")
    geometry = selected.geometry.union_all() if hasattr(selected.geometry, "union_all") else selected.geometry.unary_union
    if not geometry.buffer(1e-9).covers(BAIKONUR_POINT):
        raise SystemExit("Sovereignty geometry still excludes Baikonur lease area")
    return geometry
...
def assert_markers_inside(data: dict, geometry, bounds: tuple[float, float, float, float]) -> None:
    ...
    for label, item in markers:
        ...
        if not safe_geometry.covers(true_point):
            failures.append(f"{label}: true coordinate is outside Kazakhstan sovereignty geometry")
...
def main() -> None:
    ...
    source_note = (
        "Natural Earth 1:10m Admin 0 Sovereignty, nvkelso/natural-earth-vector "
        f"commit {SOURCE_COMMIT}, WGS84; Kazakhstan sovereignty geometry; JOURNEY ATLAS map-v3"
    )
    ...
    data.setdefault("sources", {})["map"] = (
        "Natural Earth 1:10m Admin 0 Sovereignty (nvkelso/natural-earth-vector, "
        f"commit {SOURCE_COMMIT}); Kazakhstan sovereignty geometry; WGS84."
    )
    ...
    print("Kazakhstan map rebuilt from Natural Earth 1:10m Sovereignty")
    print("Baikonur sovereignty coverage: PASS")
    print(
