...
def decode_legacy_base64(raw: str, source: str) -> bytes:
    ...
    if source.endswith(".webp.b64"):
        if len(binary) < 12 or binary[:4] != b"RIFF" or binary[8:12] != b"WEBP":
            raise ValueError(f"Decoded legacy asset is not a WebP: {source}")
    return binary
...
def materialize_encoded_asset(source: str) -> str:
    ...
    if source.endswith(".b64"):
        binary = decode_legacy_base64(source_path.read_text(encoding="utf-8"), source)
